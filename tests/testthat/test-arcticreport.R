test_that("count functions return expected numbers", {
    ...
    # baseline values were calculated from query results generated on 3/31/21
      ...
      objs <- query_objects()
      vers <- query_version_chains(objs)
      ...
      cre_count <- count_creators(vers, from = "2019-01-01", to = "2020-01-01")
      expect_equal(cre_count, 205)

      changed_count <- count_new_and_changed_datasets(vers, from = "2019-01-01", to = "2020-01-01")
...
test_that("count functions add up as expected", {
    ...
    for (i in 1:nrow(ys)){
        yr_break_new[i] <- count_new_datasets(vers, from = ys$from[i], to = ys$to[i])
        yr_break_changed[i] <- count_new_and_changed_datasets(vers, from = ys$from[i], to = ys$to[i])
        yr_break_objs[i] <- count_data_objects(vers, from = ys$from[i], to = ys$to[i])
    }

    yr_total_new <- count_new_datasets(vers, from = min(ys$from), to = max(ys$to))
    yr_total_objs <- count_data_objects(vers, from = min(ys$from), to = max(ys$to))
