...
qt <- quarters %>% 
  dplyr::filter(period %in% grep("q1", quarters$period, value = T)) # filter for quarters of interest
...
themes <- arrange(themes, -y6q1)

datatable(themes, options = list(pageLength = 15), rownames = F, height = 3000)
```
